...
import Text.Parsec.Char (char, anyChar, space, string, noneOf)
...
yearValid low high ys = all isDigit ys && low <= y && y <= high where
    y = read ys :: Int
...
hgtValid s = validSuffixWith "cm" 150 193 s || validSuffixWith "in" 59 76 s where
    validSuffixWith suffix low high s = isSuffixOf suffix s && low <= h && h <= high where
...
readPassports p = catMaybes . map (readPassport p). splitOn "" . lines

readAllValid :: (Passport -> Bool) -> IO Int
readAllValid validator = do
    text <- readFile "day04input.txt"
